...
$sql = "SELECT 
        ...
        WHERE $cateMainSQL $cateSubSQL $brandSQL $searchSQL products.is_valid = 1 $orderSQL
        LIMIT $perPage OFFSET $pageStart";

$sqlAll = "SELECT * FROM `products` WHERE $cateMainSQL $cateSubSQL $brandSQL $searchSQL `is_valid`=1 $orderSQL ";
...
<body>
				...
				<div class="">
					<div class="">
						<div class="d-flex align-items-center mb-4 px-2">
							...
							<a class="btn btn-add ms-auto px-3" href="../products/add.php">
								<i class="fa-solid fa-plus"> 新增資料</i>
							</a>
						</div>
								...
								<?php foreach ($rows as $index => $row): ?>
									<div class="list-row text-dark px-2">
										...
										<div class="control">
											...
											<button class="btn btn-sm btn-del" data-id="<?= $row["id"] ?>">
												<?= $btnDel ?>
											</button>
										</div>
									</div>
	...
	<script>
		...
		btnSearch.addEventListener("click", function () {
			...
			if (cidValue != 0) {
				window.location.href = `./index.php?&cid=${cidValue}&brand_id=${brandIDValue}&search=${query}&per_page=${perPageValue}`;
				return;
			}
			if (midValue != 0) {
				window.location.href = `./index.php?&mid=${midValue}&brand_id=${brandIDValue}&search=${query}&per_page=${perPageValue}`;
				return;
			}
			window.location.href = `./index.php?mid=${midValue}&cid=${cidValue}&brand_id=${brandIDValue}&search=${query}&per_page=${perPageValue}`;
