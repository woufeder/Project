...
<body>
    <div class="container">
        <h1>會員列表</h1>
        <span>總共 <?= $totalCount ?> 位會員</span>
        <div class="my-2">
            <a class="btn btn-primary btn-add" href="./add.php">新增會員</a>
            <a class="btn btn-primary ms-5" href="./doLogout.php">登出</a>
        </div>
        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <a class="btn btn-primary mb-3" href="./deleteIndex.php">停權會員列表</a>
        </div>
        ...
        <form method="get" id="search-form" class="d-flex justify-content-end mb-3">
            <div class="input-group">
                ...
                <select name="searchType" class="form-select" data-default-name="searchType">
                    ...
                    <option value="name" <?= ($_GET["searchType"] ?? "") == "name" ? "selected" : "" ?>>姓名</option>
                    <option value="account" <?= ($_GET["searchType"] ?? "") == "account" ? "selected" : "" ?>>帳號</option>
                    <option value="email" <?= ($_GET["searchType"] ?? "") == "email" ? "selected" : "" ?>>信箱</option>
                </select>
            ...
            <button type="button" class="btn btn-secondary ms-3" id="btn-reset">清除條件</button>
        </form>
        ...
        <div class="bg-white rounded-3 overflow-hidden shadow-sm p-2">
            ...
            <?php foreach ($rows as $index => $row): ?>
                <div class="d-flex align-items-center py-3 px-2 <?= $index === 0 ? '' : 'border-top' ?> table-row">
                    ...
                    <div class="edit">
                        <a class="btn btn-primary btn-sm" href="./view.php?id=<?= $row["id"] ?>">檢視</a>
                        <a class="btn btn-warning btn-sm" href="./update.php?id=<?= $row["id"] ?>">修改</a>
                        <button class="btn btn-danger btn-sm btn-del" data-id="<?= $row["id"] ?>">停權</button>
                    </div>
                </div>
            <?php endforeach; ?>
        </div>
